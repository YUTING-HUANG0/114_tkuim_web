...
- [ ] **架構實作 (Service Pattern)**
    - [ ] 建立 Controller/Service/Utils 資料夾結構
    - [ ] 實作統一錯誤處理 (Error Handling)
- [ ] **認證系統 (Auth)**
    - [ ] 實作 JWT 簽發 (Login/Register API)
    - [ ] 建立 JWT 驗證 Middleware (Auth Guard)
- [ ] **核心 API 開發 (部分)**
    - [ ] 實作「匿名投稿」API (Create)
    - [ ] 實作「隨機選題」演算法與 API (Read - Random)

## 📅 12/26 (Day 3): 前端基礎與認證整合
**目標**：搭建 Vue 前端環境，完成登入註冊頁面並串接後端。

- [ ] **前端初始化 (Vue + Vite)**
    - [ ] 建立 Vue 專案，安裝 Vue Router, Pinia
    - [ ] 設定 Axios (Base URL, JWT Interceptor)
    - [ ] 設置基礎 CSS / Design System (極簡風格、響應式)
- [ ] **頁面開發：認證**
    - [ ] 設計登入頁面 (Login)
    - [ ] 設計註冊頁面 (Register)
    - [ ] 串接登入/註冊 API 與 Token 管理
